...
describe('BillingTable', () => {
  it('shows consolidated billing period, sent state, paid amount and balance', () => {
    ...
    expect(screen.getByText(/Recebido: R\$ 1\.000,00/i)).toBeInTheDocument();
    expect(screen.getByText(/Saldo: R\$ 2\.000,00/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /abrir recibo/i })).toHaveAttribute('href', '/contratos-locacoes/recibos/billing-1');
  });
});
