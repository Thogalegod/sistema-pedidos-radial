...
export default function CabinePrimariaMenuPage() {
  return (
    <div className="min-h-screen bg-slate-50">
      <ContentContainer>
        <PageHeader
          breadcrumbs={[{ label: 'Central', href: '/hub' }, { label: 'Relatórios Técnicos', href: '/relatorios-tecnicos' }, { label: 'Cabine Primária' }]}
          title="Cabine Primária"
          description="Relatórios de campo, inspeção de concessionária e manutenção completa da cabine."
        />
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {modules.map((module) => {
            ...
            const content = (
              <>
                <div className={`mb-4 flex size-10 items-center justify-center rounded-lg ${module.active ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
                  <Icon size={21} />
                </div>
            ...
            return module.active ? (
              <Link
                key={module.title}
                href={module.href}
                className="group rounded-xl border border-slate-200 bg-white p-5 transition-colors hover:border-blue-400"
              >
                {content}
              </Link>
            ) : (
