...
- [x] **1B.1 — Leitores tolerantes: testes/QA concluídos e aceite humano recebido; commit/push autorizados, baseline de lint documentado. Publicação ainda precisa ser confirmada antes de M04 remota.**
- [ ] 1B.2 — Backfill idempotente (M04).
...
Prioridade atual: fechamento Git autorizado de 1A.1–1B.1 e preparação local de 1B.2. Antes da aplicação remota do backfill, confirmar a versão compatível publicada no ambiente correto e obter autorização específica para M04, conforme o master. Banco permanece 34/34 neste fechamento. As oportunidades gerais de refatoração ficam nos gates correspondentes; não constituem uma frente paralela.
